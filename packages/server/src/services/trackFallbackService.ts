...
const SUPPORTED_SOURCES: ReadonlySet<Exclude<MusicSource, 'kugou' | 'bilibili' | 'bandcamp'>> = new Set(['netease', 'tencent'])
...
export function getFallbackTargetSource(from: MusicSource): Exclude<MusicSource, 'kugou' | 'bilibili' | 'bandcamp'> | null {
...
export async function findBestAlternativeTrack(
  ...
  if (!SUPPORTED_SOURCES.has(original.source as Exclude<MusicSource, 'kugou' | 'bilibili' | 'bandcamp'>)) return null
