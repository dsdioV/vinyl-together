...
#### 两个正交的维度：IP 与身份（2026-09 实测）

香港生产环境实测发现「海外播不了」其实是**两个独立条件各缺一个**，不能笼统归因：

| 歌曲类型                     | 需要什么     | 香港直连（有登录 cookie）          | 香港中继（借用中继者身份 + 其出口 IP） | 境内直连 + 登录 |
| ---------------------------- | ------------ | ---------------------------------- | -------------------------------------- | --------------- |
| 免费歌（`pay_play=0`）       | **境内 IP**  | 常失败（实测 1 首失败 7 次）       | ✅ **救回 4 次**                       | ✅              |
| VIP 歌（`pay_play=1`）       | **会员身份** | ✅ 会员时成功（`vkey ok (plain)`） | ✅ 仅当中继者是会员                    | ✅              |
| VIP + 地区受限（如周杰伦版） | **会员身份** | ✅ 会员时成功；非会员失败          | ✅ 仅当中继者是会员                    | ✅ 会员         |

- **关键：付费/受限歌看的是「账号会员身份」，不是音质档位、也不是 IP**。生产日志时间线直接证实：同一账号非会员（`vipType=0`）时周杰伦版三通道全 `104003`；换上会员体验卡后（`vipType=1`）**香港直连**立刻 `vkey ok (plain)` 成功。详见 §5「真正决定…的是账号会员身份」。
- 早期「非绿钻只能拿 128k 档、320k 被拒」的说法是**从抓包过度外推**的错误结论——抓包只证明该账号当时无权取高码率，不能推出低码率一定可播（真实路径请求 320k，直连全拒即整体失败，不会自动降到 128k）。
- **中继同时提供「境内出口 IP」与「中继者的登录态」**：中继把香港直连拿不到的免费歌救回来（IP 维度），并且确实携带中继者身份（见 §5「中继携带登录态」）。早期文档称其为「匿名」是**错的**。
- **VIP 歌要的是身份**：香港直连虽然是香港 IP，但带房间登录 cookie 且账号有会员时，实测能拿到 VIP 歌（`vkey ok ... uin=<非0>`）。所以「登录了也没用」是错的——登录确实在用。
- **`uin` 参数本身不是凭证**：境内用真实 `uin`（不带 cookie）请求 VIP 歌仍是 `104003`，必须有 cookie。因此「只把 `uin` 填对」不能解锁 VIP；身份来自浏览器 cookie。
- **自己开中继无法解决自己的会员限制**：中继借用的是中继者自己的身份，等价于直连。要让房间播这类歌，中继者必须是会员。
...
`vipType` 由 `tencentAuthService` 从 QQ 的 `vip_login_base` 接口读取（`identity.svip ? 2 : identity.vip ? 1 : 0`），是账号真实会员状态。结论：

- **有会员 → 香港直连即可播放**（不需要中继）；**无会员 → 直连全拒**。
- **中继救不了这类歌**：中继用的是**中继者自己的**身份与出口 IP。自己开中继 = 用自己身份，等价于直连；因此非会员开中继也拿不到。
- 因此「周杰伦版播不了」是**账号权益问题**，不是代码 bug、也不是单纯的 IP 问题。免费/非会员状态下无论直连还是自开中继都不可用。

> 未验证的缺口：用户在 QQ 客户端拥有的「免费听歌时长」是否会被 `vip_login_base` 反映为 `vipType > 0`，**未测**。若不会，这类用户仍拿不到该曲，失败提示宜区分「需开通会员」而非笼统归因「海外 IP 受限」。
