...
| VIP 歌（`pay_play=1`）       | **登录身份** | ✅ 部分成功（实测 2/2）      | ✅（取决于中继者权益）                 | ✅              |
| VIP + 地区受限（如周杰伦版） | **两者都要** | ❌ 失败 8/8（登录着也不行）  | ✅ 低音质档可播（见下）                | ✅              |

- **中继同时提供「境内出口 IP」与「中继者的登录态」**：中继把香港直连拿不到的免费歌救回来（IP 维度），并且确实携带中继者身份（见 §5「中继携带登录态」）。早期文档称其为「匿名」是**错的**。
- **VIP 歌要的是身份**：香港直连虽然是香港 IP，但带房间登录 cookie 时实测能拿到 VIP 歌（`vkey ok ... uin=<非0>`）。所以「登录了也没用」是错的——登录确实在用。
- **`uin` 参数本身不是凭证**：境内用真实 `uin`（不带 cookie）请求 VIP 歌仍是 `104003`，必须有 cookie。因此「只把 `uin` 填对」不能解锁 VIP；身份来自浏览器 cookie。
- **周杰伦版（VIP + 地区受限）在香港可经中继播放，但受中继者账号权益限制**：非绿钻账号只能拿到 `M500`(128k mp3) / `C400`(m4a)；`M800`(320k) 与 `C600` 返回 104003。即「能播，但音质受限于中继者的会员等级」。
...
同一响应还显示：非绿钻账号 `M800`(320k)/`C600` 被拒（104003），但 **`M500`(128k mp3) 与 `C400`(m4a) 返回 `result:0` 并带有效 vkey**。用该 purl 在香港服务器实测 `fetch Range` = **HTTP 206**（签名不绑 IP），即中继解析出的链接可由香港服务端正常取流。
