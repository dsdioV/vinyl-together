...
| 歌曲类型                     | 需要什么     | 香港直连（有登录 cookie）    | 香港中继（境内 IP，`uin=0` 匿名） | 境内直连 + 登录 |
| ---------------------------- | ------------ | ---------------------------- | --------------------------------- | --------------- |
| 免费歌（`pay_play=0`）       | **境内 IP**  | 常失败（实测 1 首失败 7 次） | ✅ **中继救回 4 次**              | ✅              |
| VIP 歌（`pay_play=1`）       | **登录身份** | ✅ 部分成功（实测 2/2）      | ❌ 104003                         | ✅              |
| VIP + 地区受限（如周杰伦版） | **两者都要** | ❌ 失败 8/8（登录着也不行）  | ❌ 匿名                           | ✅              |

- **中继的真实作用是提供一个境内出口 IP**，不是「转发 cookie」：中继维持 `uin='0'` 匿名，却能把香港直连拿不到的**免费歌**救回来。
- **VIP 歌要的是身份**：香港直连虽然是香港 IP，但带房间登录 cookie 时实测能拿到 VIP 歌（`vkey ok ... uin=<非0>`）。所以「登录了也没用」是错的——登录确实在用。
- **`uin` 参数本身不是凭证**：境内用真实 `uin`（不带 cookie）请求 VIP 歌仍是 `104003`，必须有 cookie。因此「只把 `uin` 填对」不能解锁 VIP。
- 中继的 `<script>` 请求是**从本页面域到 `u.y.qq.com` 的跨站子资源请求**，若上游 cookie 为 `SameSite=Lax`（现代默认）则**不会携带**；上游匿名访问也不下发 `Set-Cookie`，故「中继是否带登录态」**未证实**，不要当作既成事实。

> 待立项：让香港也能播「VIP + 地区受限」类歌曲。备选方案为「服务器接一个境内出口」（凭证不出服务端，仅需几 KB 的 API 请求走出口，音频仍由客户端直连 CDN），优于「服务器下载音频再分发」（后者引入存储/版权/带宽成本，且 5 MB 下载会突破房间锁的 12 s 预算）。
